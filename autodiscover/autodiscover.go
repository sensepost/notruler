...
func GetMapiHTTP(email, autoURLPtr string, resp *utils.AutodiscoverResp) (*utils.AutodiscoverResp, string, string, error) {
	//var resp *utils.AutodiscoverResp
	var err error
	var rawAutodiscover, autodiscurl string

	if autoURLPtr == "" && resp == nil {
		utils.Trace.Println("Retrieving MAPI/HTTP info")
		...
		resp, rawAutodiscover, autodiscurl, err = MAPIDiscover(maildomain)
	} else if resp == nil {
		resp, rawAutodiscover, autodiscurl, err = MAPIDiscover(autoURLPtr)
	}
	...
	return resp, rawAutodiscover, autodiscurl, nil
}

//GetRPCHTTP exports the RPC details for RPC/HTTP
func GetRPCHTTP(email, autoURLPtr string, resp *utils.AutodiscoverResp) (*utils.AutodiscoverResp, string, string, string, bool, string, error) {
	//var resp *utils.AutodiscoverResp
	var err error
	var rawAutodiscover, autodiscurl string
	...
	if autoURLPtr == "" && resp == nil {
		...
		maildomain := email[lastBin+1:]
		resp, rawAutodiscover, autodiscurl, err = Autodiscover(maildomain)
	} else if resp == nil {
		resp, rawAutodiscover, autodiscurl, err = Autodiscover(autoURLPtr)
	}
	...
	return resp, rawAutodiscover, RPCURL, user, ntlmAuth, autodiscurl, nil
}
...
func CheckCache(email string) *utils.AutodiscoverResp {
	...
	utils.Trace.Println("Found cached Autodiscover record. Using this (use --nocache to force new lookup)")
...
func Autodiscover(domain string) (*utils.AutodiscoverResp, string, string, error) {
...
func MAPIDiscover(domain string) (*utils.AutodiscoverResp, string, string, error) {
	//set transport
	if SessionConfig.Proxy == "" {
		Transport = http.Transport{
			TLSClientConfig: &tls.Config{InsecureSkipVerify: SessionConfig.Insecure},
...
func autodiscover(domain string, mapi bool) (*utils.AutodiscoverResp, string, string, error) {
	...
	if err != nil {
		...
		if m, _ := regexp.Match("illegal base64", []byte(err.Error())); m == true {
			client = http.Client{Transport: InsecureRedirectsO365{User: SessionConfig.User, Pass: SessionConfig.Pass, Insecure: SessionConfig.Insecure}}
			resp, err = client.Do(req)
			if err != nil {
				return nil, "", "", err
			}
	...
	if resp.StatusCode == 200 {
		...
		return &autodiscoverResp, string(body), autodiscoverURL, nil
	}
